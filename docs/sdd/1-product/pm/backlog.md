...
- [ ] `P3` Favoritar — precisa de tabela nova `presentation_favorite` (`presentation_id`, `user_id`, `created_at`, unique nos dois primeiros) — decidido explicitamente **não** ser um boolean solto em `presentation`, já nasce por-usuário pra não precisar redesenhar quando colaboração existir. Tab "Favoritas" já existe na UI (desabilitada) esperando esse campo
...
- [x] `P3` Restaurar da lixeira / excluir definitivamente (1 item ou em lote) — implementado. `presentationService().restore()` (volta status pra `active`, não guarda o status anterior ao trashar), `restoreAll()`/`emptyTrash()` (loop sobre `findMany` filtrado por `status=trash`, reaproveita o mesmo `remove()` do job de retenção pra exclusão real). Rotas: `POST .../[id]/restore`, `DELETE .../[id]/permanent`, `POST .../trash/restore`, `DELETE .../trash`. Wired no dropdown do card (restore direto, deletePermanently com modal de confirmação — `AppPresentationDeletePermanentlyModal`) e no `AppPresentationsTrashToolbar`
- [x] `P3` Filtros (recentes/apresentações/diagramas/favoritas) — `Tabs` com `onValueChange` real, viram `tab` na query de `GET /presentations` (server-side desde a paginação, ver item abaixo): "Todas"/"Lixeira" trocam o escopo, "Recentes" filtra por `createdAt` nas últimas 24h (janela fixa, não configurável), "Apresentações"/"Diagramas" filtram por `presentation_entry.type`. "Favoritas" continua desabilitada (depende do item de Favoritar acima). Removida a tab "Criadas por você" — app ainda não tem modelo multi-usuário, não haveria o que diferenciar de "Todas"
- [x] `P3` Busca — input sempre visível na listagem completa (trocou o antigo botão-combobox `AppPresentationsSearch` nessa tela), com debounce de 300ms, vira `q` na query do servidor, refina dentro do escopo da tab ativa
- [x] `P3` Filtro de visibilidade (público/privado) — `Select` na listagem completa, vira `visibility` na query do servidor, refina dentro do escopo da tab ativa (mesmo padrão da busca)
- [x] `P2` Paginação da listagem — implementada com cursor (`createdAt`+`id`), não offset (evita item repetido/pulado se a lista mudar entre páginas) — decisão completa e raciocínio em `decisions.md`. UI: botão "Carregar mais" + "Voltar ao topo", ambos no fim do grid
- [ ] `P3` `AppPresentationsSearch` (combobox, usado em community/templates) busca só na primeira página (20 itens) desde a paginação — antes buscava em todas as presentations do usuário. Corrigir exige o combobox mandar o texto digitado como `q` pro servidor em vez do filtro client-side atual do `CommandInput`
...
**UI — Ciclo 2**
- [ ] `P3` Padronizar ícones por recurso — hoje cada página usa um ícone diferente pro mesmo recurso (ex: `presentations` é `GalleryVerticalEnd` no `AppNavRail`, mas `Presentation` na section de stats/empty state de `/app/presentations`). Definir um mapa único ícone↔recurso antes de mais páginas nascerem com escolhas divergentes
