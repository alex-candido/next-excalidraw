...
import { AppStartTemplatesHeader } from "@/components/app/start/templates/app-start-templates-header";
...
export function AppStartTemplates({
  className,
  ...props
    ...
    <>
      <LayoutSection className="first:pt-6 md:first:pt-8 md:pb-16">
        <LayoutContainer className="justify-center">
          <div
            className={cn(
              "app-start-templates w-full max-w-4xl flex flex-col gap-4",
              className,
            )}
            {...props}
          >
            <AppStartTemplatesHeader />

            {ITEMS.length === 0 ? (
              <div className="app-start-templates-empty">
                <Muted className="text-sm">{t("empty")}</Muted>
              </div>
            ) : (
              <div className="app-start-templates-grid grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {ITEMS.map((item) => (
                  <AppPresentationCard
                    key={item.id}
                    title={item.title}
