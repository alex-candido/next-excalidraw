...
// (extrativo, ver lib/utils/text/) pra ~6 frases antes de qualquer outra
...
function finalizeText(text: string): string {
  const summarized = textUtils().summarize(text, SUMMARY_SENTENCE_COUNT)
...
export function attachmentUtils() {
  ...
  async function buildContext(rows: AttachmentRow[]): Promise<AttachmentContext[]> {
    ...
    for (const row of rows) {
      ...
      if (row.type === AttachmentType.file && row.content) {
        ...
        try {
          const text = await fileUtils().extractText(row.content, row.mimeType ?? "")
